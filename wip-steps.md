...
3. OpenSSL Configuration Request edition
   ...
   3. Section `[ req_dn ]`
      - Unlike in the root and intermediate configurations, instead of telling OpenSSL what DNs to prompt for and the user friendly names and their defaults. This section is used to define what the DN values are.
   4. Section `[ req_ext ]`
...
1. Intermediate Certificate  
   ...
   2. Generate the intermediate key and signing request
      ...
      4. run the following command while in your intermediate ca folder `openssl req -new -config intermediate-ca-req.cnf -newkey -rand private/.rand -days <number of days> -keyout private/intermediate.key -out certs/intermediate.crt`.  
      To break this down, you are asking the OpenSSL req module to create a `-new` request using the `-config` located at the file you just saved, you want to generate a `-newkey` as specified either in the config or by a preceeding argument. You want to seed this certificate using the `-rand` file located under `private/.rand` as generated in section two. And finally you are going to output you key, `-keyout`, to the path as defined in your `openssl.cnf` and `-out`put you certifcate to the path as defined in your `openssl.cnf`
   2. 
